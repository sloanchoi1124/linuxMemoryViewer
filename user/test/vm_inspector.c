...
int main(int argc, char **argv)
{
	...
	if (syscall(245, pid, fake_pgd_base, fake_pmd_base,
	...
	while (current_va <= end_vaddr) {
		...
		if ((fake_pgd_base + cur_pgd_index * ENTRY_SIZE)
		    >= fake_pgd_max)
			goto next_va;

		cur_fake_pmd = *((unsigned long *)
				 (fake_pgd_base + cur_pgd_index * ENTRY_SIZE));
		if (cur_fake_pmd == 0)
			goto next_va;
		...
		cur_fake_pte = *((unsigned long *)
				 (cur_fake_pmd + cur_pmd_index * ENTRY_SIZE));
		if (cur_fake_pte == 0)
			goto next_va;
		...
		table_entry = *((unsigned long *)
				(cur_fake_pte + cur_pte_index * ENTRY_SIZE));
		...
		printf("0x%lx\t0x%lx\t%d\t%d\t%d\t%d\t%d\n",
		       current_va, cur_pa,
		       pte_young(table_entry),
		       pte_file(table_entry),
		       pte_dirty(table_entry),
		       pte_read_only(table_entry),
		       pte_uxn(table_entry));
		current_va += 4096;
		continue;
next_va:
